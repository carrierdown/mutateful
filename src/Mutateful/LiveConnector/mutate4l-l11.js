...
// currently unused, will be needed if a message queue is to be used
function processQueue() {
    ...
    if (dataToSend[0] === resetUpdatedInternallyIdentifier) {
...
function getClipDataAsBytes(liveObject, trackNo, clipNo) {
    ...
    return [].slice.call(resultBuffer); // convert to regular array
}
...
function sendDataToServer(cmdSignifier, payload) {
    outlet(0, [cmdSignifier].concat(payload));
}
